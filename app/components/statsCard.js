...
export default function StatsCard({ icon, number, disc }) {
    ...
    return (
        <div className='flex flex-col justify-center items-center font-bold gap-2'>
            ...
            <p className='max-md:text-[1.5em] md:text-[2em] bg-gradient-to-r from-[#1F73D4] via-[#7C39D4] to-[#1F73D4] bg-clip-text text-transparent'>{number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",").toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
