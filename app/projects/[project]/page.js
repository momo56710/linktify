...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetchData();
    progress.current
      ? setBarWidth(progress.current.offsetWidth)
      : setBarWidth(0);
  }, [progress.current , ]);
  ...
  startups.map((e) =>
    e.title.replaceAll(" ", "-").toLowerCase() == pathName ? (startup = e) : ""
  );

  // getting funding percentage
  const percentage = Math.trunc(
    (Number(startup.funded) * 100) / Number(startup.goal)
  );

  // comapring percentage to progress bar
  const handleBacker = async (close) => {
    startup.backers.push({
      backer: { name: user.displayName, email: user.email },
  ...
  return (
    <div className="max-w-[1600px] m-auto my-10 max-md:text-[12px] md:p-10">
        ...
        <div className="flex gap-10 max-md:flex-col mb-8">
          ...
          <div className="md:flex-1 my-auto">
            <div className="m-auto max-md:mx-5">
              <p className="text-center mb-4 font-bold text-[2em] max-md:text-[1.5em]">
                Stats
              </p>
              <ProgressBar
                completed={percentage}
                customLabel=" "
                className="h-5 relative max-w-[100%] rounded-[5px] overflow-hidden mr-10"
                bgColor="#2271B9"
              />

              <p className="mb-4 text-[1.5em]">
                {" "}
                <span className="font-bold">{percentage}%</span> of the goal
              </p>
              <p className="text-[1.5em] font-bold text-[#2271B9]">
                {startup.funded
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                  .toString()
              ...
              <div className="flex gap-4 justify-center items-center mt-5 max-md:flex-col">
                {user ? (
                  <Popup
                    trigger={
                      <p className="bg-[#000218] cursor-pointer text-white px-5 py-3 font-bold rounded-full uppercase max-md:w-full text-center">
                        Back this project
                      </p>
                    }
                    modal
                      ...
                      <div className="modal p-5">
                        ...
                        <div className="content w-full flex flex-col gap-5">
                          <input
                            ...
                            className="border bg-transparent p-3 rounded-[10px] w-full"
                            onChange={(e) => {
                              setBackingAmount(e.target.value);
                            }}
                          />
                          <p
                            className="bg-[#2271B9] cursor-pointer  hover:bg-blue-600 text-white gird place-content-center p-2 rounded-md mb-2 text-center flex items-center content-center"
                            onClick={() => {
                              backingAmount < 1000
                                ? toast(
                                    "backing amount can't be less than 1000DA"
                                  )
                                : handleBacker(close);
                            }}
                          >
                  ...
                  <p
                    className="bg-[#000218] cursor-pointer text-white px-5 py-3 font-bold rounded-full uppercase max-md:w-full text-center"
                    onClick={() => {
                ...
                <p className="bg-[#E7EBF2] text-[#000218] px-5 py-3 font-bold rounded-full uppercase max-md:w-full text-center">
                  share
                </p>
              </div>
            </div>
      ...
      <p className="text-center text-[6em] my-10 font-bold text-[#001623]">
        our story
      </p>
      <div className="flex flex-col gap-[4em] m-10  max-md:items-center">
        {startup.stories.map((e, i) => (
          <div
            key={i}
            className="flex max-md:flex-col  gap-10 p-10 rounded-2xl bg-white md:divide-x max-md:divide-y max-md:items-center"
          >
            <img
              src={e.cover}
              className="max-w-[300px] rounded-2xl aspect-square"
            />
